...
class PastExamViewSet(viewsets.ModelViewSet):
    ...
    #             # Ensure each non-image column is converted to string
    ...
    #                 if question_image_data:
    #                     question_text = None  # If image exists, skip text
    ...
    #                     col_letter = get_column_letter(df.columns.get_loc(option_key) + 1) if option_key in df.columns else ""
    ...
    #                     # if created:
    #                     #     # If it's a new option, link it to the PastExamQuestionOption
    ...
    #                     #     )
    #                     # else:
    #                         # If the option already exists, link it to the PastExamQuestionOption
    #                     PastExamQuestionOption.objects.get_or_create(
    #                         exam=past_exam,
    #                         question=question,
    #                         option=option_obj
    #                     )
    ...
    #                     if option_image_data:
    #                         filename = f"option_image_{uuid.uuid4().hex}.png"
    #                         image_file = self.save_image_to_field(option_image_data, filename)
    #                         if image_file:
    ...
    #         if has_answer: required_columns.append("answer")
    #         if has_difficulty: required_columns.append("difficulty")
    #         if has_category: required_columns.append("category")
    ...
    #                 if has_difficulty and row["difficulty"]:
    #                     try:
    #                         difficulty_level = int(float(row["difficulty"]))
    #                     except ValueError:
    #                         return {"error": f"Invalid difficulty level at row {index + 2}"}
    ...
    #                 question = None
    #                 if question_text:
    #                     question, created = Question.objects.get_or_create(text=question_text, defaults=question_data)
    #                     if not created:
    #                         updated_questions += 1
    #                         if not question.category and category:
    #                             question.category = category
    ...
    #                 # Explanation
    #                 explanation_text = str(row.get("explanation", "")).strip() if has_explanation else ""
    #                 explanation_cell = f"{get_column_letter(df.columns.get_loc('explanation') + 1)}{index + 2}" if has_explanation else None
    #                 explanation_image_data = self.get_image_data_from_map(image_map.get(explanation_cell)) if explanation_cell else None

    #                 if explanation_image_data:
    #                     question.explanation = None
    #                     filename = f"explanation_image_{uuid.uuid4().hex}.png"
    #                     image_file = self.save_image_to_field(explanation_image_data, filename)
    #                     if image_file:
    #                         question.explanation_image.save(image_file.name, image_file)
    ...
    #                 # Create PastExamQuestion without unique constraint
    ...
    #                 options = []
    #                 for option_key in option_columns:
    #                     option_text = str(row.get(option_key, "")).strip() if pd.notna(row.get(option_key)) else None
    #                     col_letter = get_column_letter(df.columns.get_loc(option_key) + 1) if option_key in df.columns else ""
    #                     option_cell = f"{col_letter}{index + 2}"
    #                     option_image_data = self.get_image_data_from_map(image_map.get(option_cell))
    ...
    #                     option_obj, _ = QuestionOption.objects.get_or_create(question=question, text=option_text)
